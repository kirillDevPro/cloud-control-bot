...
MESSAGES: dict[str, str] = {
    ...
    'settings.section_language_desc': 'язык интерфейса и уведомлений',
    ...
    'settings.section_balance': '💵 Уведомления о балансе',
    'settings.section_balance_desc': 'порог уведомления о низком балансе',
    'settings.balance_status_on': '🔔 <b>Уведомления о балансе:</b> Вкл',
    'settings.balance_status_off': '🔕 <b>Уведомления о балансе:</b> Выкл',
    'settings.balance_threshold_current': '💵 <b>Порог:</b> ${value:.2f}',
    'settings.balance_threshold_hint': '<i>Уведомление приходит, когда баланс провайдера опускается ниже этого значения.</i>',
    'settings.balance_choose': 'Выберите пресет или задайте своё значение:',
    'settings.balance_btn_on': '🔔 Включить',
    'settings.balance_btn_off': '🔕 Выключить',
    'settings.balance_btn_custom': '✏️ Своё значение',
    'settings.balance_prompt_title': '✏️ <b>Своё значение порога</b>',
    'settings.balance_prompt_body': 'Пришлите новый порог в USD числом (например, <code>500</code>).',
    'settings.balance_invalid': '❌ Это не похоже на сумму. Пришлите положительное число, например <code>500</code>, или нажмите «Отмена».',
    'settings.balance_enabled_toast': '🔔 Уведомления о балансе включены',
    'settings.balance_disabled_toast': '🔕 Уведомления о балансе выключены',
    'settings.balance_threshold_set_toast': '✅ Порог установлен: ${value:.2f}',
    'settings.balance_not_saved_toast': '⚠️ Применено сейчас, но не удалось сохранить (после перезапуска может сброситься).',
    'settings.balance_not_saved_notice': '⚠️ Применено сейчас, но не удалось сохранить (после перезапуска может сброситься).',
    'settings.balance_cancelled': '✖️ Ввод отменён.',
    # admin
    ...
    'notif.low_balance.settings_hint': 'ℹ️ <i>Порог можно изменить или отключить уведомления в Настройки → Уведомления о балансе.</i>',
    ...
    'notif.server_removed.body': (
        ...
        '🗑️ Статистика какое-то время сохраняется — если сервер вернётся, история уцелеет.'
    ),
    'notif.sync_suspicious.title': '⚠️ <b>Подозрительный ответ API: {provider}</b>',
    'notif.sync_suspicious.body': (
        'API вернул пустой список серверов, хотя локально известно {count}. '
        'Ничего не удалено — мониторинг и статистика сохранены.'
    ),
    'notif.sync_suspicious.footer': (
        'Если список правда пуст и останется таким около часа, серверы будут удалены автоматически.'
    ),
    'notif.sync_mass_removal.title': '⚠️ <b>Массовое удаление отложено: {provider}</b>',
    'notif.sync_mass_removal.body': (
        'Из ответа API разом пропало {count} серверов из {total}. '
        'Удаление отложено до подтверждения на следующем цикле синхронизации.'
    ),
    'notif.sync_mass_removal.footer': (
        'Если это сбой провайдера, серверы просто вернутся и ничего не будет потеряно.'
    ),
    # alert
    ...
    # svc (уведомления сервис-проверок)
    'svc.check_down.title': '🔴 <b>Проверка не прошла</b>',
    'svc.check_up.title': '🟢 <b>Проверка восстановлена</b>',
    'svc.ssl_expiring.title': '🔒 <b>Сертификат скоро истечёт</b>',
    'svc.ssl_invalid.title': '🔒 <b>Проблема с сертификатом</b>',
    'svc.check.body': '<b>{server}</b>\n{check}',
    'svc.rollup.title': '🔴 <b>Множественные сбои проверок</b>',
    # checks (UI сервис-проверок по серверу)
    'checks.button': '🩺 Проверки ({count})',
    'checks.list.title': '🩺 <b>Сервис-проверки — {server}</b>',
    'checks.list.empty': 'Проверок пока нет. Добавьте ниже.',
    'checks.list.prompt': 'Нажмите проверку для управления или добавьте новую.',
    'checks.detail.title': '<b>{desc}</b>',
    'checks.detail.enabled': '✅ Включена',
    'checks.detail.disabled': '⏸ Отключена',
    'checks.detail.stats': '📊 Аптайм {pct}% • {ok}/{total} ок (24ч)',
    'checks.detail.latency': '⚡ Средняя задержка ~{ms} мс',
    'checks.detail.no_data': '📊 Данных пока нет.',
    'checks.detail.ssl_days': '⏳ Истекает через {days} дн',
    'checks.detail.ssl_until': '📅 Действителен до {date}',
    'checks.detail.ssl_problem': '⚠️ {detail}',
    'checks.kb.add': '➕ Добавить проверку',
    'checks.kb.cancel': '✖️ Отмена',
    'checks.kb.tcp': '🔌 TCP',
    'checks.kb.http': '🌐 HTTP',
    'checks.kb.ssl': '🔒 SSL',
    'checks.kb.enable': '✅ Включить',
    'checks.kb.disable': '⏸ Отключить',
    'checks.kb.delete': '🗑️ Удалить',
    'checks.kb.delete_yes': '🗑️ Да, удалить',
    'checks.kb.delete_no': '↩️ Нет',
    'checks.prompt.choose_type': '➕ <b>Новая проверка</b>\nВыберите тип проверки.',
    'checks.prompt.port': '🔌 <b>TCP-проверка</b>\nОтправьте номер порта (1–65535).',
    'checks.prompt.ssl_port': '🔒 <b>SSL-проверка</b>\nОтправьте порт или «-» для 443.',
    'checks.prompt.url': '🌐 <b>HTTP-проверка</b>\nОтправьте полный URL (http:// или https://).',
    'checks.prompt.keyword': '🌐 <b>Ключевое слово</b>\nОтправьте слово, которое должно быть в теле ответа, или «-» чтобы пропустить.',
    'checks.prompt.status': '🌐 <b>Ожидаемый статус</b>\nОтправьте ожидаемый код HTTP или «-» для 200.',
    'checks.prompt.warn_days': '🔒 <b>Окно предупреждения</b>\nОтправьте, за сколько дней до истечения предупреждать, или «-» для значения по умолчанию.',
    'checks.invalid.port': '⚠️ Введите порт от 1 до 65535.',
    'checks.invalid.url': '⚠️ Введите корректный URL http:// или https://.',
    'checks.invalid.keyword': '⚠️ Слишком длинное слово (макс. 200 символов).',
    'checks.invalid.status': '⚠️ Введите код статуса от 100 до 599.',
    'checks.invalid.warn_days': '⚠️ Введите число дней от 1 до 365.',
    'checks.added': '✅ Проверка добавлена.',
    'checks.deleted': '🗑️ Проверка удалена.',
    'checks.not_saved': '⚠️ Сохранено только в памяти (ошибка записи на диск).',
    'checks.not_saved_notice': '⚠️ <b>Проверка добавлена (только в памяти — ошибка записи на диск).</b>',
    'checks.cancelled': 'Отменено.',
    'checks.delete_confirm': '🗑️ <b>Удалить эту проверку?</b>',
    'checks.not_found': 'Проверка не найдена.',
}
...
PLURALS: dict[str, list[str]] = {
    ...
    'svc.rollup.count': [
        '{n} проверка не проходит',
        '{n} проверки не проходят',
        '{n} проверок не проходят',
    ],
}
