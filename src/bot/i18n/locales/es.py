...
MESSAGES: dict[str, str] = {
    ...
    'settings.section_balance': '💵 Alertas de saldo',
    'settings.section_balance_desc': 'umbral de aviso de saldo bajo',
    'settings.balance_status_on': '🔔 <b>Alertas de saldo:</b> Activadas',
    'settings.balance_status_off': '🔕 <b>Alertas de saldo:</b> Desactivadas',
    'settings.balance_threshold_current': '💵 <b>Umbral:</b> ${value:.2f}',
    'settings.balance_threshold_hint': '<i>Se envía un aviso cuando el saldo de un proveedor cae por debajo de este valor.</i>',
    'settings.balance_choose': 'Elige un preajuste o define tu propio valor:',
    'settings.balance_btn_on': '🔔 Activar',
    'settings.balance_btn_off': '🔕 Desactivar',
    'settings.balance_btn_custom': '✏️ Valor personalizado',
    'settings.balance_prompt_title': '✏️ <b>Umbral personalizado</b>',
    'settings.balance_prompt_body': 'Envía el nuevo umbral en USD como número (por ejemplo, <code>500</code>).',
    'settings.balance_invalid': '❌ No es un importe válido. Envía un número positivo como <code>500</code>, o pulsa Cancelar.',
    'settings.balance_enabled_toast': '🔔 Alertas de saldo activadas',
    'settings.balance_disabled_toast': '🔕 Alertas de saldo desactivadas',
    'settings.balance_threshold_set_toast': '✅ Umbral fijado en ${value:.2f}',
    'settings.balance_not_saved_toast': '⚠️ Aplicado por ahora, pero no se pudo guardar (podría restablecerse tras un reinicio).',
    'settings.balance_not_saved_notice': '⚠️ Aplicado por ahora, pero no se pudo guardar (podría restablecerse tras un reinicio).',
    'settings.balance_cancelled': '✖️ Entrada cancelada.',
    # admin
    ...
    'notif.low_balance.settings_hint': 'ℹ️ <i>Cambia el umbral o desactiva los avisos en Ajustes → Alertas de saldo.</i>',
    ...
    'notif.server_removed.body': (
        ...
        '\n'
        '⛔ Monitoreo detenido.\n'
        '🗑️ Las estadísticas se conservan un tiempo: si el servidor vuelve, mantiene su historial.'
    ),
    'notif.sync_suspicious.title': '⚠️ <b>Respuesta sospechosa de la API: {provider}</b>',
    'notif.sync_suspicious.body': (
        'La API devolvió una lista de servidores vacía aunque localmente se conocen {count}. '
        'No se eliminó nada: el monitoreo y las estadísticas están intactos.'
    ),
    'notif.sync_suspicious.footer': (
        'Si la lista está realmente vacía y sigue así durante aproximadamente una hora, los '
        'servidores se eliminarán automáticamente.'
    ),
    'notif.sync_mass_removal.title': '⚠️ <b>Eliminación masiva aplazada: {provider}</b>',
    'notif.sync_mass_removal.body': (
        'De la respuesta de la API desaparecieron de golpe {count} servidores de {total}. '
        'La eliminación se aplaza hasta que el siguiente ciclo de sincronización lo confirme.'
    ),
    'notif.sync_mass_removal.footer': (
        'Si es un fallo del proveedor, los servidores volverán y no se perderá nada.'
    ),
    # alert
    ...
    # svc (notificaciones de comprobaciones de servicio)
    'svc.check_down.title': '🔴 <b>Comprobación fallida</b>',
    'svc.check_up.title': '🟢 <b>Comprobación recuperada</b>',
    'svc.ssl_expiring.title': '🔒 <b>El certificado caduca pronto</b>',
    'svc.ssl_invalid.title': '🔒 <b>Problema con el certificado</b>',
    'svc.check.body': '<b>{server}</b>\n{check}',
    'svc.rollup.title': '🔴 <b>Múltiples comprobaciones fallando</b>',
    # checks (UI de comprobaciones de servicio por servidor)
    'checks.button': '🩺 Comprobaciones ({count})',
    'checks.list.title': '🩺 <b>Comprobaciones de servicio — {server}</b>',
    'checks.list.empty': 'Aún no hay comprobaciones. Añade una abajo.',
    'checks.list.prompt': 'Toca una comprobación para gestionarla, o añade una.',
    'checks.detail.title': '<b>{desc}</b>',
    'checks.detail.enabled': '✅ Activada',
    'checks.detail.disabled': '⏸ Desactivada',
    'checks.detail.stats': '📊 Disponibilidad {pct}% • {ok}/{total} ok (24h)',
    'checks.detail.latency': '⚡ Latencia media ~{ms} ms',
    'checks.detail.no_data': '📊 Aún no hay datos.',
    'checks.detail.ssl_days': '⏳ Caduca en {days} d',
    'checks.detail.ssl_until': '📅 Válido hasta {date}',
    'checks.detail.ssl_problem': '⚠️ {detail}',
    'checks.kb.add': '➕ Añadir comprobación',
    'checks.kb.cancel': '✖️ Cancelar',
    'checks.kb.tcp': '🔌 TCP',
    'checks.kb.http': '🌐 HTTP',
    'checks.kb.ssl': '🔒 SSL',
    'checks.kb.enable': '✅ Activar',
    'checks.kb.disable': '⏸ Desactivar',
    'checks.kb.delete': '🗑️ Eliminar',
    'checks.kb.delete_yes': '🗑️ Sí, eliminar',
    'checks.kb.delete_no': '↩️ No',
    'checks.prompt.choose_type': '➕ <b>Añadir comprobación</b>\nElige el tipo de comprobación.',
    'checks.prompt.port': '🔌 <b>Comprobación TCP</b>\nEnvía el número de puerto (1–65535).',
    'checks.prompt.ssl_port': '🔒 <b>Comprobación SSL</b>\nEnvía el puerto, o «-» para 443.',
    'checks.prompt.url': '🌐 <b>Comprobación HTTP</b>\nEnvía la URL completa (http:// o https://).',
    'checks.prompt.keyword': '🌐 <b>Palabra clave</b>\nEnvía una palabra que deba aparecer en el cuerpo de la respuesta, o «-» para omitir.',
    'checks.prompt.status': '🌐 <b>Estado esperado</b>\nEnvía el código de estado HTTP esperado, o «-» para 200.',
    'checks.prompt.warn_days': '🔒 <b>Ventana de aviso</b>\nEnvía los días antes de la caducidad para avisar, o «-» para el valor por defecto.',
    'checks.invalid.port': '⚠️ Introduce un puerto entre 1 y 65535.',
    'checks.invalid.url': '⚠️ Introduce una URL http:// o https:// válida.',
    'checks.invalid.keyword': '⚠️ Palabra demasiado larga (máx. 200 caracteres).',
    'checks.invalid.status': '⚠️ Introduce un código de estado entre 100 y 599.',
    'checks.invalid.warn_days': '⚠️ Introduce un número de días entre 1 y 365.',
    'checks.added': '✅ Comprobación añadida.',
    'checks.deleted': '🗑️ Comprobación eliminada.',
    'checks.not_saved': '⚠️ Guardado solo en memoria (falló la escritura en disco).',
    'checks.not_saved_notice': '⚠️ <b>Comprobación añadida (solo en memoria — falló la escritura en disco).</b>',
    'checks.cancelled': 'Cancelado.',
    'checks.delete_confirm': '🗑️ <b>¿Eliminar esta comprobación?</b>',
    'checks.not_found': 'Comprobación no encontrada.',
}
...
PLURALS: dict[str, list[str]] = {
    ...
    'svc.rollup.count': [
        '{n} comprobación de servicio está fallando',
        '{n} comprobaciones de servicio están fallando',
    ],
}
