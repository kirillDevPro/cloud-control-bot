...
MESSAGES: dict[str, str] = {
    ...
    'settings.section_language_desc': 'мова інтерфейсу та сповіщень',
    ...
    'settings.section_balance': '💵 Сповіщення про баланс',
    'settings.section_balance_desc': 'поріг сповіщення про низький баланс',
    'settings.balance_status_on': '🔔 <b>Сповіщення про баланс:</b> Увімк.',
    'settings.balance_status_off': '🔕 <b>Сповіщення про баланс:</b> Вимк.',
    'settings.balance_threshold_current': '💵 <b>Поріг:</b> ${value:.2f}',
    'settings.balance_threshold_hint': '<i>Сповіщення надходить, коли баланс провайдера опускається нижче цього значення.</i>',
    'settings.balance_choose': 'Оберіть пресет або задайте власне значення:',
    'settings.balance_btn_on': '🔔 Увімкнути',
    'settings.balance_btn_off': '🔕 Вимкнути',
    'settings.balance_btn_custom': '✏️ Власне значення',
    'settings.balance_prompt_title': '✏️ <b>Власне значення порога</b>',
    'settings.balance_prompt_body': 'Надішліть новий поріг у USD числом (наприклад, <code>500</code>).',
    'settings.balance_invalid': '❌ Це не схоже на суму. Надішліть додатне число, наприклад <code>500</code>, або натисніть «Скасувати».',
    'settings.balance_enabled_toast': '🔔 Сповіщення про баланс увімкнено',
    'settings.balance_disabled_toast': '🔕 Сповіщення про баланс вимкнено',
    'settings.balance_threshold_set_toast': '✅ Поріг встановлено: ${value:.2f}',
    'settings.balance_not_saved_toast': '⚠️ Застосовано зараз, але не вдалося зберегти (після перезапуску може скинутися).',
    'settings.balance_not_saved_notice': '⚠️ Застосовано зараз, але не вдалося зберегти (після перезапуску може скинутися).',
    'settings.balance_cancelled': '✖️ Введення скасовано.',
    # admin
    ...
    'notif.low_balance.settings_hint': 'ℹ️ <i>Поріг можна змінити або вимкнути сповіщення в Налаштування → Сповіщення про баланс.</i>',
    ...
    'notif.server_removed.body': (
        ...
        '\n'
        '⛔ Моніторинг зупинено.\n'
        '🗑️ Статистика деякий час зберігається — якщо сервер повернеться, історія вціліє.'
    ),
    'notif.sync_suspicious.title': '⚠️ <b>Підозрілива відповідь API: {provider}</b>',
    'notif.sync_suspicious.body': (
        'API повернув порожній список серверів, хоча локально відомо {count}. '
        'Нічого не видалено — моніторинг і статистику збережено.'
    ),
    'notif.sync_suspicious.footer': (
        'Якщо список справді порожній і залишиться таким близько години, сервери буде видалено '
        'автоматично.'
    ),
    'notif.sync_mass_removal.title': '⚠️ <b>Масове видалення відкладено: {provider}</b>',
    'notif.sync_mass_removal.body': (
        'З відповіді API за раз зникло {count} серверів із {total}. '
        'Видалення відкладено до підтвердження на наступному циклі синхронізації.'
    ),
    'notif.sync_mass_removal.footer': (
        'Якщо це збій провайдера, сервери просто повернуться і нічого не буде втрачено.'
    ),
    # alert
    ...
    # svc (сповіщення сервіс-перевірок)
    'svc.check_down.title': '🔴 <b>Перевірка не пройшла</b>',
    'svc.check_up.title': '🟢 <b>Перевірка відновлена</b>',
    'svc.ssl_expiring.title': '🔒 <b>Сертифікат скоро закінчиться</b>',
    'svc.ssl_invalid.title': '🔒 <b>Проблема із сертифікатом</b>',
    'svc.check.body': '<b>{server}</b>\n{check}',
    'svc.rollup.title': '🔴 <b>Множинні збої перевірок</b>',
    # checks (UI сервіс-перевірок по серверу)
    'checks.button': '🩺 Перевірки ({count})',
    'checks.list.title': '🩺 <b>Сервіс-перевірки — {server}</b>',
    'checks.list.empty': 'Перевірок поки немає. Додайте нижче.',
    'checks.list.prompt': 'Натисніть перевірку для керування або додайте нову.',
    'checks.detail.title': '<b>{desc}</b>',
    'checks.detail.enabled': '✅ Увімкнена',
    'checks.detail.disabled': '⏸ Вимкнена',
    'checks.detail.stats': '📊 Аптайм {pct}% • {ok}/{total} ок (24г)',
    'checks.detail.latency': '⚡ Середня затримка ~{ms} мс',
    'checks.detail.no_data': '📊 Даних поки немає.',
    'checks.detail.ssl_days': '⏳ Закінчується через {days} дн',
    'checks.detail.ssl_until': '📅 Дійсний до {date}',
    'checks.detail.ssl_problem': '⚠️ {detail}',
    'checks.kb.add': '➕ Додати перевірку',
    'checks.kb.cancel': '✖️ Скасувати',
    'checks.kb.tcp': '🔌 TCP',
    'checks.kb.http': '🌐 HTTP',
    'checks.kb.ssl': '🔒 SSL',
    'checks.kb.enable': '✅ Увімкнути',
    'checks.kb.disable': '⏸ Вимкнути',
    'checks.kb.delete': '🗑️ Видалити',
    'checks.kb.delete_yes': '🗑️ Так, видалити',
    'checks.kb.delete_no': '↩️ Ні',
    'checks.prompt.choose_type': '➕ <b>Нова перевірка</b>\nОберіть тип перевірки.',
    'checks.prompt.port': '🔌 <b>TCP-перевірка</b>\nНадішліть номер порту (1–65535).',
    'checks.prompt.ssl_port': '🔒 <b>SSL-перевірка</b>\nНадішліть порт або «-» для 443.',
    'checks.prompt.url': '🌐 <b>HTTP-перевірка</b>\nНадішліть повний URL (http:// або https://).',
    'checks.prompt.keyword': '🌐 <b>Ключове слово</b>\nНадішліть слово, яке має бути в тілі відповіді, або «-» щоб пропустити.',
    'checks.prompt.status': '🌐 <b>Очікуваний статус</b>\nНадішліть очікуваний код HTTP або «-» для 200.',
    'checks.prompt.warn_days': '🔒 <b>Вікно попередження</b>\nНадішліть, за скільки днів до закінчення попереджати, або «-» для значення за замовчуванням.',
    'checks.invalid.port': '⚠️ Введіть порт від 1 до 65535.',
    'checks.invalid.url': '⚠️ Введіть коректний URL http:// або https://.',
    'checks.invalid.keyword': '⚠️ Занадто довге слово (макс. 200 символів).',
    'checks.invalid.status': '⚠️ Введіть код статусу від 100 до 599.',
    'checks.invalid.warn_days': '⚠️ Введіть кількість днів від 1 до 365.',
    'checks.added': '✅ Перевірку додано.',
    'checks.deleted': '🗑️ Перевірку видалено.',
    'checks.not_saved': '⚠️ Збережено лише в пам’яті (помилка запису на диск).',
    'checks.not_saved_notice': '⚠️ <b>Перевірку додано (лише в пам’яті — помилка запису на диск).</b>',
    'checks.cancelled': 'Скасовано.',
    'checks.delete_confirm': '🗑️ <b>Видалити цю перевірку?</b>',
    'checks.not_found': 'Перевірку не знайдено.',
}
...
PLURALS: dict[str, list[str]] = {
    ...
    # svc (сповіщення сервіс-перевірок)
    'svc.ssl.days_left': [
        '⏳ Закінчується через {n} день',
        '⏳ Закінчується через {n} дні',
        '⏳ Закінчується через {n} днів',
    ],
    'svc.rollup.count': [
        '{n} перевірка не проходить',
        '{n} перевірки не проходять',
        '{n} перевірок не проходять',
    ],
}
