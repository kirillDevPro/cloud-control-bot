...
MESSAGES: dict[str, str] = {
    ...
    'settings.section_balance': '💵 Balance alerts',
    'settings.section_balance_desc': 'low-balance notification threshold',
    'settings.balance_status_on': '🔔 <b>Balance alerts:</b> On',
    'settings.balance_status_off': '🔕 <b>Balance alerts:</b> Off',
    'settings.balance_threshold_current': '💵 <b>Threshold:</b> ${value:.2f}',
    'settings.balance_threshold_hint': '<i>An alert is sent when a provider balance drops below this value.</i>',
    'settings.balance_choose': 'Pick a preset or set your own value:',
    'settings.balance_btn_on': '🔔 Turn on',
    'settings.balance_btn_off': '🔕 Turn off',
    'settings.balance_btn_custom': '✏️ Custom value',
    'settings.balance_prompt_title': '✏️ <b>Custom threshold</b>',
    'settings.balance_prompt_body': 'Send the new threshold in USD as a number (for example, <code>500</code>).',
    'settings.balance_invalid': '❌ That is not a valid amount. Send a positive number like <code>500</code>, or tap Cancel.',
    'settings.balance_enabled_toast': '🔔 Balance alerts on',
    'settings.balance_disabled_toast': '🔕 Balance alerts off',
    'settings.balance_threshold_set_toast': '✅ Threshold set to ${value:.2f}',
    'settings.balance_not_saved_toast': '⚠️ Applied for now, but could not be saved (it may reset after a restart).',
    'settings.balance_not_saved_notice': '⚠️ Applied for now, but could not be saved (it may reset after a restart).',
    'settings.balance_cancelled': '✖️ Input cancelled.',
    # admin
    ...
    'notif.low_balance.settings_hint': 'ℹ️ <i>Change the threshold or turn alerts off in Settings → Balance alerts.</i>',
    ...
    # svc (service-check notifications)
    'svc.check_down.title': '🔴 <b>Service check failed</b>',
    'svc.check_up.title': '🟢 <b>Service check recovered</b>',
    'svc.ssl_expiring.title': '🔒 <b>Certificate expiring soon</b>',
    'svc.ssl_invalid.title': '🔒 <b>Certificate problem</b>',
    'svc.check.body': '<b>{server}</b>\n{check}',
    'svc.rollup.title': '🔴 <b>Multiple service checks failing</b>',
    # checks (per-server service-check UI)
    'checks.button': '🩺 Checks ({count})',
    'checks.list.title': '🩺 <b>Service checks — {server}</b>',
    'checks.list.empty': 'No checks configured yet. Add one below.',
    'checks.list.prompt': 'Tap a check to manage, or add one.',
    'checks.detail.title': '<b>{desc}</b>',
    'checks.detail.enabled': '✅ Enabled',
    'checks.detail.disabled': '⏸ Disabled',
    'checks.detail.stats': '📊 Uptime {pct}% • {ok}/{total} ok (24h)',
    'checks.detail.latency': '⚡ Avg latency ~{ms} ms',
    'checks.detail.no_data': '📊 No data yet.',
    'checks.detail.ssl_days': '⏳ Expires in {days} d',
    'checks.detail.ssl_until': '📅 Valid until {date}',
    'checks.detail.ssl_problem': '⚠️ {detail}',
    'checks.kb.add': '➕ Add check',
    'checks.kb.cancel': '✖️ Cancel',
    'checks.kb.tcp': '🔌 TCP',
    'checks.kb.http': '🌐 HTTP',
    'checks.kb.ssl': '🔒 SSL',
    'checks.kb.enable': '✅ Enable',
    'checks.kb.disable': '⏸ Disable',
    'checks.kb.delete': '🗑️ Delete',
    'checks.kb.delete_yes': '🗑️ Yes, delete',
    'checks.kb.delete_no': '↩️ No',
    'checks.prompt.choose_type': '➕ <b>Add a check</b>\nChoose the check type.',
    'checks.prompt.port': '🔌 <b>TCP check</b>\nSend the port number (1–65535).',
    'checks.prompt.ssl_port': '🔒 <b>SSL check</b>\nSend the port, or «-» for 443.',
    'checks.prompt.url': '🌐 <b>HTTP check</b>\nSend the full URL (http:// or https://).',
    'checks.prompt.keyword': '🌐 <b>Keyword</b>\nSend a word that must appear in the response body, or «-» to skip.',
    'checks.prompt.status': '🌐 <b>Expected status</b>\nSend the expected HTTP status code, or «-» for 200.',
    'checks.prompt.warn_days': '🔒 <b>Warning window</b>\nSend days-before-expiry to warn, or «-» for the default.',
    'checks.invalid.port': '⚠️ Enter a port between 1 and 65535.',
    'checks.invalid.url': '⚠️ Enter a valid http:// or https:// URL.',
    'checks.invalid.keyword': '⚠️ Keyword too long (max 200 characters).',
    'checks.invalid.status': '⚠️ Enter a status code between 100 and 599.',
    'checks.invalid.warn_days': '⚠️ Enter a number of days between 1 and 365.',
    'checks.added': '✅ Check added.',
    'checks.deleted': '🗑️ Check deleted.',
    'checks.not_saved': '⚠️ Saved in memory only (disk write failed).',
    'checks.not_saved_notice': '⚠️ <b>Check added (in memory only — disk write failed).</b>',
    'checks.cancelled': 'Cancelled.',
    'checks.delete_confirm': '🗑️ <b>Delete this check?</b>',
    'checks.not_found': 'Check not found.',
}
...
PLURALS: dict[str, list[str]] = {
    ...
    'svc.rollup.count': [
        '{n} service check is failing',
        '{n} service checks are failing',
    ],
}
